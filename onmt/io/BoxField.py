...
class BoxField(RawField):
    ...
    Attributes:
        sequential: Whether the datatype represents sequential datasets. If False,
            no tokenization is applied. Default: True.
        use_vocab: Whether to use a Vocab object. If False, the datasets in this
        ...
        tensor_type: The torch.Tensor class that represents a batch of examples
            of this kind of datasets. Default: torch.LongTensor.
    ...
    def build_vocab(self, *args, **kwargs):
        ...
        Arguments:
            Positional arguments: Dataset objects or other iterable datasets
    ...
    def numericalize(self, arr, device=None, train=True):
        ...
        if self.include_lengths and not isinstance(arr, tuple):
            raise ValueError("Field has include_lengths set to True, but "
                             "input datasets is not a tuple of "
                             "(datasets batch, batch lengths).")
        ...
        if self.use_vocab:
            ...
            # the datasets is sequential, since it's unclear how to coerce padding tokens
            # to a numeric type.
            if not self.sequential:
...
class CharField(RawField):
    ...
    def __init__(self, sequential=True, use_vocab=True, init_token=None,
                 eos_token=None, fix_length=None, tensor_type=torch.LongTensor,
                 preprocessing=None, postprocessing=None, lower=False,
                 tokenize=(lambda s: s.split()), include_lengths=True,
                 batch_first=False, pad_token="<pad>", unk_token="<unk>",
                 pad_first=False, left_pad=False):
        self.sequential = sequential
        self.use_vocab = use_vocab
        self.init_token = init_token
        self.eos_token = eos_token
    ...
    def pad(self, minibatch):
        ...
        #     return minibatch
        if self.fix_length is None:
            max_len = max([max([len(item) for item in x]) for x in minibatch])
        else:
            max_len = self.fix_length + (
                self.init_token, self.eos_token).count(None) - 2
        ...
        for x in minibatch:
            if self.pad_first:
                ...
                padded_lens = []
                for item in x:
                    # padded_item = item[:max_len] + [self.pad_token] * max(0, max_len - len(item))
                    # left padding
                    if self.left_pad:
                        padded_item = [self.pad_token] * max(0, max_len - len(item)) + item[:max_len]
                        padded_lens.append(len(padded_item) - len(item[:max_len]))
                        #  assert False
                    else:
                        # right padding
                        padded_item = item[:max_len] + [self.pad_token] * max(0, max_len - len(item))
                        padded_lens.append(len(item))
                    padded_x.append(padded_item)
                padded.append(padded_x)
                lengths.append(padded_lens)
    ...
    def numericalize(self, arr, device=None, train=True):
        ...
        if self.use_vocab:
            if self.sequential:
                ...
                # arr = [[self.vocab.stoi[x] for x in ex] for ex in arr
                arr = [[[self.vocab.stoi[item] for item in x] for x in ex] for ex in arr]
        ...
        bsz, n_record, max_len = arr.size()
        arr = arr.view(bsz*n_record, max_len)
        lengths = lengths.view(bsz*n_record)
