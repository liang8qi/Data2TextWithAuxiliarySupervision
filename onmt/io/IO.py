...
def get_fields(data_type, n_src_features, n_tgt_features, cal_indices=False):
    """
    Args:
    ...
    """
    if data_type == 'text':
        return TextDataset.get_fields(n_src_features, n_tgt_features, cal_indices=cal_indices)
    elif data_type == 'img':
...
def load_fields_from_vocab(vocab, data_type="text", cal_indices=False):
    """
    ...
    fields = get_fields(data_type, n_src_features, n_tgt_features, cal_indices=cal_indices)
...
def make_features(batch, side, data_type='text'):
    """
    Args:
        batch (Variable): a batch of source or target datasets.
        side (str): for source or for target.
        data_type (str): type of the source input.
            Options are [text|img|audio].
    Returns:
...
def build_dataset(fields, data_type, src_path, src_hist_path, src_path2, tgt_path2, src_dir=None,
                  src_seq_length=0, tgt_seq_length=0,
                  src_seq_length_trunc=0, tgt_seq_length_trunc=0,
                  dynamic_dict=True, sample_rate=0,
                  window_size=0, window_stride=0, window=None,
                  normalize_audio=True, use_filter_pred=True, cal_indices=False, hier_meta=None):
    ...
    src_examples_iter, num_src_feats = \
        _make_examples_nfeats_tpl(data_type, src_path, src_dir,
                                  ...
                                  window, normalize_audio, "src", cal_indices=cal_indices, hier_meta=hier_meta)

    tgt_examples_iter, num_tgt_feats = \
        TextDataset.make_text_examples_nfeats_tpl(
            tgt_path2, tgt_seq_length_trunc, "tgt")

    if data_type == 'text':
        dataset = TextDataset(fields=fields, src_examples_iter=src_examples_iter, tgt_examples_iter=tgt_examples_iter,
                              num_src_feats=num_src_feats,
    ...
    for path in train_dataset_files:
        ...
        for ex in dataset.examples:
            for k in fields:
                ...
                if val is not None and k.endswith("_char"):
                    # val is tuple of tuple
                    # assert False
                    val = [tmp_char for tmp_exp in val for tmp_char in tmp_exp]
...
def _make_examples_nfeats_tpl(data_type, src_path, src_dir,
                              src_seq_length_trunc, sample_rate,
                              window_size, window_stride,
                              window, normalize_audio, src="src", cal_indices=False, hier_meta=None):
    """
    ...
    if data_type == 'text':
        src_examples_iter, num_src_feats = \
            TextDataset.make_text_examples_nfeats_tpl(
                src_path, src_seq_length_trunc, src, cal_indices=cal_indices, hier_meta=hier_meta)
