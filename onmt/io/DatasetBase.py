...
class ONMTDatasetBase(torchtext.data.Dataset):
    ...
    Internally it initializes an `torchtext.datasets.Dataset` object with
    the following attributes:

     `examples`: a sequence of `torchtext.datasets.Example` objects.
     `fields`: a dictionary associating str keys with `torchtext.datasets.Field`
    ...
    def _construct_example_fromlist(self, data, fields):
        """
        Args:
            data: the datasets to be set as the value of the attributes of
                the to-be-created `Example`, associating with respective
                `Field` objects with same key.
            fields: a dict of `torchtext.datasets.Field` objects. The keys
        ...
        for (name, field), val in zip(fields, data):
            # val is tuple / or tuple of tuple for chars
