...
class GlobalSelfAttention(nn.Module):
    ...
    * Bahdanau Attention (mlp):
    ...
    """
    def __init__(self, dim, coverage=False, attn_type="dot", attn_hidden=0, no_gate=False, no_gate_bias=False,
                 no_fusion=False):
        ...
        if self.no_gate:
            ...
            if no_fusion:
                in_dim = dim
            self.linear_out = nn.Sequential(nn.Linear(in_dim, dim, bias=no_gate_bias), nn.Tanh())
        else:
            self.linear_out = nn.Linear(2*dim, dim, bias=out_bias)
    ...
    def forward(self, input, memory_bank, memory_lengths=None, coverage=None):
        ...
        if self.attn_type in ["general", "dot"]:
            ...
            if not self.no_gate:
                ...
                # vectors_vision(attn_h[0].to('cpu'))
                # assert False
