...
class CopyGenerator(nn.Module):
    ...
    * :math:`p_{copy}` the probility of copying a word instead.
    ...
    def __init__(self, input_size, tgt_dict):
        ...
        self.padding_idx = tgt_dict.stoi[onmt.io.PAD_WORD]
...
class CopyGeneratorLossCompute(onmt.Loss.LossComputeBase):
    ...
    def __init__(self, generator, tgt_vocab,
        ...
        self.switch_loss_criterion = nn.BCELoss(reduction=reduction)
        # self.switch_loss_criterion = nn.BCELoss(size_average=False)
        # self.switch_loss_criterion = nn.BCEWithLogitsLoss(reduction=reduction)
    ...
    def _compute_loss(self, batch, output, target, copy_attn, align, ptrs):
        ...
        # if normalize_by_length == False, sum(lens*bsz)
        # ne, if a[i] == num return False, else True
        ...
        # normalize_by_length default is False
        if self.normalize_by_length:
            ...
            switch_loss = switch_loss.view(-1, batch.batch_size).sum(0)
            # Divide by length of each sequence and sum
            loss = torch.div(loss, tgt_lens).sum()

            switch_loss = torch.div(switch_loss, tgt_lens).sum()
            assert False
        else:
            loss = loss.sum()
