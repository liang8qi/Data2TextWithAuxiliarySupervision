...
class HierarchicalEncoder(EncoderBase):
    ...
    def __init__(self, meta, num_layers, embeddings, emb_size, attn_hidden,
                 rnn_type, bidirectional, hidden_size, dropout=0.0, attn_type="general",
                 coverage_attn=False, two_dim_score=None, hier_history_seq_type=None,
                 hier_history_seq_window=None, hier_num_layers=None, hier_hist_attn_type=None,
                 hier_hist_attn_pos_type=None, use_hier_hist=True):
        ...
        if use_hier_hist:
            if hier_history_seq_type == "sa" or hier_history_seq_type == "SA":
                ...
                self.hier_history_seq_attn_layer = onmt.modules.GlobalSelfAttention(emb_size, coverage=False,
                                                                                    attn_type=hier_hist_attn_type,
                                                                                    attn_hidden=attn_hidden,
                                                                                    no_gate=True, no_gate_bias=True)
            else:
        ...
        self.use_hier_hist = use_hier_hist
    ...
    def obtainHierRep(self, emb, start_id, end_id, num, num_tensor, kw_num, kw_num_tensor, batch, hist_emb,
                      hist_start_id, hist_end_id):
        ...
        if self.use_hier_hist:
            ...
            home_ply_rep_hist = home_ply_rep_hist.transpose(0, 1).contiguous().view(-1, self.hier_history_seq_window,
                                                                                    home_ply_rep_hist.size(2)).transpose(0, 1).contiguous()
        ...
        # input: (batch*ply_num, kw_num, hidden), (batch*ply_num, kw_num, hidden), (batch*ply_num)
        # output: (kw_num, batch*ply_num, hidden)
        ...
        home_ply_memory_bank, col_rep = self.getTwoDimRep(home_ply_memory_bank, home_ply_rep, batch, num_tensor, home_ply_rep_hist)

        # (kw_num*ply_num, bsz, hsz)
        home_ply_row_rep = home_ply_memory_bank.mean(0).view(batch, -1, home_ply_memory_bank.size(2)).transpose(0, 1).contiguous()

        return home_ply_row_rep, home_ply_memory_bank, home_ply_rep, col_rep
    ...
    def getTwoDimRep(self, rnn_output, memory_bank, batch_size, memory_lengths, home_ply_rep_hist):
        ...
        col_rep, _ = self.col_attn(col_memory_bank.transpose(0, 1).contiguous(),
        ...
        assert rep_concat.size(3) == row_dim3
        num_cated = 3 if self.use_hier_hist else 2

        rep_view = rep_concat.view(-1, num_cated, row_dim3)
        score_view = two_dim_weight.view(-1, num_cated).unsqueeze(1)

        return torch.bmm(score_view, rep_view).squeeze(1).view(row_dim1, row_dim2, row_dim3), output_col_rep
    ...
    def forward(self, src, lengths=None, encoder_state=None, memory_lengths=None):
        ...
        hist_home_ply_start = None
        hist_home_ply_end = None
        if self.use_hier_hist:
            # 4
            hist_home_ply_start = self.meta['home_ply_start']
            # 4 + 284*win_size -1
            hist_home_ply_end = hist_home_ply_start + ((self.meta['home_ply_end'] - self.meta['home_ply_start'] + 1)
                                                       * self.hier_history_seq_window) - 1

        home_ply_row_rep, home_ply_memory_bank, \
        home_embed, home_col_rep = self.obtainHierRep(emb, self.meta['home_ply_start'],
                                                      self.meta['home_ply_end'],
                                                      self.meta['home_ply_num'],
                                                      self.home_ply_length,
                                                      self.meta['home_ply_kw_num'],
                                                      self.home_ply_num_kw_num, batch, hist_emb,
                                                      hist_home_ply_start, hist_home_ply_end)
        hist_vis_ply_start = None
        hist_vis_ply_end = None
        if self.use_hier_hist:
            hist_vis_ply_start = hist_home_ply_end + 1
            hist_vis_ply_end = hist_vis_ply_start + ((self.meta['vis_ply_end'] - self.meta['vis_ply_start'] + 1)
                                                     * self.hier_history_seq_window) - 1

        vis_ply_row_rep, vis_ply_memory_bank, \
        vis_embed, vis_col_rep = self.obtainHierRep(emb, self.meta['vis_ply_start'],
                                                    self.meta['vis_ply_end'],
                                                    self.meta['vis_ply_num'],
                                                    self.vis_ply_length,
                                                    self.meta['vis_ply_kw_num'],
                                                    self.vis_ply_num_kw_num, batch, hist_emb,
                                                    hist_vis_ply_start, hist_vis_ply_end)
        hist_team_start = None
        hist_team_end = None
        if self.use_hier_hist:
            hist_team_start = hist_vis_ply_end + 1
            hist_team_end = hist_team_start + ((self.meta['team_end'] - self.meta['team_start'] + 1)
                                               * self.hier_history_seq_window) - 1
        
            assert hist_team_end == hist_emb.size(0) - 1

        team_row_rep, team_memory_bank, \
        team_embed, team_col_rep = self.obtainHierRep(emb, self.meta['team_start'],
                                                      self.meta['team_end'],
                                                      self.meta['team_num'], self.team_length,
                                                      self.meta['team_kw_num'], self.team_kw_num,
                                                      batch, hist_emb, hist_team_start,
                                                      hist_team_end)
        
        total_row_rep = torch.cat((home_ply_row_rep, vis_ply_row_rep, team_row_rep), 0)

        total_row_rep, _ = self.row_attn(total_row_rep.transpose(0, 1).contiguous(), total_row_rep.transpose(0, 1),
        ...
        mem_bank_as_orig = torch.cat((special_rep,
                                      torch.cat([tmp_i.transpose(0, 1).contiguous().view(batch, -1, tmp_i.size(2)).transpose(0, 1)
                                                 for tmp_i in (home_ply_memory_bank, vis_ply_memory_bank, team_memory_bank)], 0)), 0)
        ...
        return encoder_final, memory_bank, (home_embed, vis_embed, team_embed), (home_col_rep, vis_col_rep, team_col_rep)

    def only_col_encoding(self, src, lengths=None, encoder_state=None, memory_lengths=None):
        if isinstance(src, tuple) and isinstance(src[0], tuple) and len(src) == 2:
            src, hist_src = src
        else:
            hist_src = None
        ...
        if self.use_hier_hist:
            hist_emb = self.dropout(self.hist_embeddings(hist_src))
            assert (self.hier_history_seq_window * (
                        self.meta["tuple_num"] - (self.meta["special_end"] - self.meta["special_start"] + 1)) + (
                                self.meta["special_end"] - self.meta["special_start"] + 1)) == hist_emb.size(0)
        ...
        _, _, home_embed, home_col_rep = self.obtainHierRep(emb, self.meta['home_ply_start'],
                                                self.meta['home_ply_end'],
                                                self.meta['home_ply_num'],
                                                self.home_ply_length,
                                                self.meta['home_ply_kw_num'],
                                                self.home_ply_num_kw_num, batch,
                                                hist_emb, hist_home_ply_start, hist_home_ply_end)
        ...
        _, _, vis_embed, vis_col_rep = self.obtainHierRep(emb, self.meta['vis_ply_start'],
                                               self.meta['vis_ply_end'],
                                               self.meta['vis_ply_num'],
                                               self.vis_ply_length,
                                               self.meta['vis_ply_kw_num'],
        ...
        _, _, team_embed, team_col_rep = self.obtainHierRep(emb, self.meta['team_start'],
                                                self.meta['team_end'],
                                                self.meta['team_num'], self.team_length,
                                                self.meta['team_kw_num'], self.team_kw_num,
                                                batch, hist_emb, hist_team_start,
        ...
        return (home_embed, vis_embed, team_embed), (home_col_rep, vis_col_rep, team_col_rep)
...
class RNNEncoder(EncoderBase):
    ...
    def forward(self, src, lengths=None, encoder_state=None):
        ...
        if lengths is not None and not self.no_pack_padded_seq:
            # Lengths datasets is wrapped inside a Variable.
            if self.sort_data:
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None):
        ...
        for i, emb_t in enumerate(emb.split(1)):
            ...
            decoder_outputs = decoder_outputs + [decoder_output]
            attns["std"] = attns["std"] + [p_attn]

            # Update the coverage attention.
            if self._coverage:
                coverage = coverage + p_attn \
                    if coverage is not None else p_attn
                attns["coverage"] = attns["coverage"] + [coverage]
            ...
            if self._copy and not self._reuse_copy_attn:
                ...
                attns["copy"] = attns["copy"] + [copy_attn]
            elif self._copy:
...
class NMTModel(nn.Module):
    ...
    """
    def __init__(self, encoder, decoder, num_ranking_decoder=None, imp_ranking_decoder=None, multigpu=False):
        self.multigpu = multigpu
        super(NMTModel, self).__init__()
        self.encoder = encoder
        self.decoder = decoder
        is_ptr_decoder = False
        if num_ranking_decoder is not None:
            self.num_ranking_decoder = num_ranking_decoder
            is_ptr_decoder = True

        if imp_ranking_decoder is not None:
            self.imp_ranking_decoder = imp_ranking_decoder
            is_ptr_decoder = True
        self.is_ptr_decoder = is_ptr_decoder
    ...
    def forward(self, src, tgt, lengths, dec_state=None):
        ...
        enc_final, memory_bank, col_reps, row_reps, entity_reps = self.encoder(src, lengths)

        """
        ptr_attns = None
        if self.is_ptr_decoder:
            home_attns = self.ptr_decoder.training(enc_outputs=home_col_rep, enc_lens=None)
            vis_attns = self.ptr_decoder.training(enc_outputs=vis_col_rep, enc_lens=None)
            team_attns = self.ptr_decoder.training(enc_outputs=team_col_rep, enc_lens=None)
            ptr_attns = [home_attns, vis_attns, team_attns]
        """
        enc_state = \
        ...
        return decoder_outputs, attns, dec_state, memory_bank[2] if isinstance(memory_bank, tuple) else memory_bank, \
               col_reps, row_reps, entity_reps

    def encoding(self, src, lengths):
        # _, _, home_col_rep, vis_col_rep, team_col_rep = self.encoder(src, lengths)
        row_tuple = None
        # col_tuple, row_tuple = self.encoder.only_table_encoding(src, lengths)
        # col_tuple = self.encoder.only_col_encoding(src, lengths)
        entity_reps = None
        col_tuple, row_tuple, entity_reps = self.encoder.only_col_and_row_encoding(src, lengths)

        return col_tuple, row_tuple, entity_reps

    def col_sorted(self, col_reps, batch):
        col_home_indices = self.change_shape_for_col(col_reps[0], batch.home_col_indices)
        col_vis_indices = self.change_shape_for_col(col_reps[1], batch.vis_col_indices)
        col_team_indices = self.change_shape_for_col(col_reps[2], batch.team_col_indices)

        home_col_rep, vis_col_rep, team_col_rep = col_reps

        home_sorted, vis_sorted, team_sorted, argmaxs = self.num_ranking_decoder(home_col_rep, col_home_indices,
                                                                             vis_col_rep, col_vis_indices,
                                                                             team_col_rep, col_team_indices)
        sorted_list = [home_sorted, vis_sorted, team_sorted]
        indices_list = [col_home_indices, col_vis_indices, col_team_indices]
        return sorted_list, indices_list, argmaxs

    def row_sorted(self, row_reps, batch):
        home_row_rep, vis_row_rep, team_row_rep = row_reps

        row_home_indices = self.change_shape_for_row(home_row_rep, batch.home_row_indices)
        row_vis_indices = self.change_shape_for_row(vis_row_rep, batch.vis_row_indices)
        row_team_indices = self.change_shape_for_row(team_row_rep, batch.team_row_indices)

        home_sorted, vis_sorted, team_sorted, argmaxs = self.imp_ranking_decoder(home_row_rep, row_home_indices,
                                                                             vis_row_rep, row_vis_indices,
                                                                             team_row_rep, row_team_indices)
        sorted_list = [home_sorted, vis_sorted, team_sorted]
        indices_list = [row_home_indices, row_vis_indices, row_team_indices]
        return sorted_list, indices_list, argmaxs
    ...
    @staticmethod
    def change_shape_for_row(row_rep, indices):
        """

        :param row_rep: (kw_num, bsz, row_num, hsz)
        :param indices: (bsz, kw_num*row_num)
        :return:
        """
        kw_num, bsz, ply_num, hsz = row_rep.size()
        assert kw_num * ply_num == indices.size(-1), "{}: {}".format(kw_num * ply_num, indices.size(-1))

        indices = indices.view(bsz, ply_num, kw_num)
        indices = indices.contiguous().view(-1, kw_num)
